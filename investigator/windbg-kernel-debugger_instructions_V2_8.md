...
Phase 8 的最終輸出，**必須固定依照以下 4 段順序產生**：
...
```
[一句白話結論]
<例如：系統不是自己藍屏，而是因為系統碟檔案 I/O 被安全元件拖慢，最後卡死後被人為強制關機。>
...
- 至少要有 3 條對照
- 每一條都必須是「一句白話」對應到「至少一個具體 WinDbg 證據」
- 禁止只寫抽象結論，不附命令或輸出結果
...
若是 `0x1C8`, `0x161`, live dump, manually initiated dump 這類「事後採樣」案例，白話因果鏈第一行必須先說清楚：

```
這個 bugcheck 不是原始故障，而是系統已經卡住後留下的現場
```
...
```
[THREE-BUCKET CONCLUSION]
已證明：
- <只能放 dump 內已被具體證據支持的事>

高度懷疑：
- <可以放最可疑 driver / component，但要保留推定語氣>

尚未直接證明：
- <仍需 ETW / live repro / verifier 才能封閉的最後一哩>
```
...
若使用者追問「可以講更直白嗎？」或「我沒聽懂」，下次輸出必須自動降階：

- 先用 3 到 5 句白話重講
- 每句只表達一個因果
- 暫停使用 stack function 名稱，除非用來做白話對照
- 優先回答「先發生什麼、後發生什麼、誰最可疑、為什麼不是別人」
